...
class _BanStatusCardViewState extends State<BanStatusCardView> with AutomaticKeepAliveClientMixin {
  ...
  var banCardsStore = Get.put(BanCardStore());
  ...
  //   const banStatusCardIdsKey = 'ban_status:card_ids';
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        Obx(() =>
        AnimatedOpacity(
          opacity: banCardsStore.pageStatus.value == PageStatus.success ? 1 : 0,
          duration: const Duration(milliseconds: 300),
          child: SingleChildScrollView(
            padding: const EdgeInsets.symmetric(horizontal: 4),
            child: Column(
              children: banCardsStore.group.value.keys
                  .map((key) => Column(
                          ...
                          Card(
                            ...
                            child: Container(
                              ...
                              child: GridView.builder(
                                  ...
                                  itemCount: banCardsStore.group?.value[key]!.length,
                                  gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                                      crossAxisCount: 5, childAspectRatio: 0.57, crossAxisSpacing: 6),
                                  itemBuilder: (context, _index) {
                                    return MdCardItemView2(
                                      id: banCardsStore.group.value[key]![_index].oid,
                                      mdCard: banCardsStore.group.value[key]![_index],
                                      onTap: (card) => handleTapCardItem(banCardsStore.group.value[key]!, _index),
                                    );
                                  }),
                            ),
                          ),
        ...
        Obx(
          () => banCardsStore.pageStatus.value == PageStatus.loading ? const Positioned.fill(child: Center(child: Loading())) : SizedBox(),
        )
      ],
    );
  }
